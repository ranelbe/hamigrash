...
export function MatchCard({ id, home, away, status, scheduledAt, homeGoals, awayGoals, venue, href }: Props) {
  const showScore = status === 'live' || status === 'finished';
  const linkHref = href ?? `/matches/${id}`;
  return (
    <Link href={linkHref} className="block group">
      <div className="rounded-xl2 bg-white dark:bg-ink-800 border border-ink-100 dark:border-ink-700 hover:border-pitch-300 dark:hover:border-pitch-700 transition-colors px-5 py-4">
