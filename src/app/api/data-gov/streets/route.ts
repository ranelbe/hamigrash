...
// GET /api/data-gov/streets?city=תל אביב יפו&q=דיזנגוף  →  { results: string[] }
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const r of records) {
      ...
      if (!name) continue;
      // Filter client-side too — CKAN's `q` is full-text and can return
      // matches from other fields (e.g. matching the city name).
      if (q && !name.includes(q)) continue;
      set.add(name);
    }
    const results = Array.from(set).sort((a, b) => a.localeCompare(b, 'he')).slice(0, 15);
