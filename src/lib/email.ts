...
function htmlBody(inv: Invitation, url: string) {
  return `<!doctype html><html dir="rtl" lang="he"><body style="font-family:system-ui,Arial;padding:24px;background:#f8fafc;color:#0f172a">
  <div style="max-width:520px;margin:0 auto;background:white;border-radius:16px;padding:32px;box-shadow:0 10px 30px -10px rgba(15,23,42,.18)">
    <h1 style="margin:0 0 12px;font-size:22px">הוזמנת ל־הַמִּגְרָשׁ ⚽</h1>
    ...
    <p style="margin:24px 0 0;font-size:12px;color:#64748b">ההזמנה תקפה עד ${new Date(inv.expires_at).toLocaleDateString('he-IL')}. יש להתחבר עם חשבון Google של ${inv.email}.</p>
  </div></body></html>`;
}
